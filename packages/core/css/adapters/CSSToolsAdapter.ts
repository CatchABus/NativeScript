...
export class CSSToolsAdapter extends AbstractCSSAdapter {
	declare protected readonly _ast: CssStylesheetAST;
	...
	override createCSSDeclarations(nodes: (CssAtRuleAST | CssDeclarationAST)[]): NSCssDeclaration[] {
		...
		for (const decl of nodes) {
			if (this.isDeclaration(decl)) {
				...
				const value = decl.value ? decl.value.substring(0, importantIdx).trim() : decl.value;
